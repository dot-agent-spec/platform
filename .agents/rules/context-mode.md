...
Reading to **edit** → read_file correct. 
Reading to **understand architecture, relationships or general codebase context** → USE `graphify` (e.g., `query_graph` ou CLI). ESTA É A SUA FERRAMENTA PRIMÁRIA E ABSOLUTA.
Reading to **extract specific raw data, count, or parse logs** → use `mcp__context-mode__ctx_execute_file(path, language, code)` as a secondary fallback.

### grep / search (large results)
For codebase searches, architecture questions, or finding where things are defined → USE `graphify` ALWAYS.
For isolated data parsing (e.g., counting lines in a CSV or filtering error logs) → Use `mcp__context-mode__ctx_execute(language: "javascript", code: "...")` in sandbox.
