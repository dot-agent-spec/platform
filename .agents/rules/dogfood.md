---
description: dogfood/ holds dated, write-once DSL usability snapshots — never spec truth. The guardrail against citing one as current behavior, plus the rules a generating agent follows.
paths: ["dogfood/**"]
---

## dogfood/ — point-in-time DSL usability snapshots

For each skill or workflow, an agent models it as `.behavior` / `.description` and records where the
language helped or got in the way. The goal is to feel the spec from an author's seat **at the moment of
generation** — not to maintain living documentation.
...
The same applies to what a dogfood *names*: a snapshot may model a skill or workflow that has since been
replaced or removed. That is expected — snapshots are never retro-corrected.
...
5. Lint the `.behavior`/`.description` with `@dot-agent/compiler`; **never** `pack` or run them (no kernel
   here).
6. **Quarantine skill output.** Running the skill under test may write real files into the live tree (an
   example ADR in `project/adr/`, an example RFC in `project/rfcs/`, etc.). After capturing your findings,
   **move every generated artifact into `dogfood/<this-skill>/examples/`, mirroring its original path** —
   e.g. an example ADR → `dogfood/new-adr/examples/adr/<file>` — or delete it. **Never leave skill output
   in the live `project/adr/` / `project/rfcs/` / `examples/` folders.** These `examples/` are throwaway
   proof and are git-ignored — do not commit them.
