...
---
- The MCP tool `dot_agent_init` takes an optional `force` boolean, so an MCP caller can overwrite the files `init` would otherwise refuse to replace ([#47](https://github.com/dot-agent-spec/platform/issues/47)).
