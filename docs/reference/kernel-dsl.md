...
| **UI** | `apply_css`, `remove_css` | UI layer — must apply changes to the DOM/renderer |
...
**What JS must do:** Instantiate a new `AgentDSLKernel` with the referenced behavior (or delegate to the agent runtime), run it to completion, and notify the parent FSM:
...
The `.behavior` text could not be parsed. Log `message` and prevent further execution.
...
export function useAgentDSLKernel(handlers: AgentDSLKernelHandlers) {
  const engineRef = useRef<import("@dot-agent/kernel-dsl").AgentDSLKernel | null>(null);

  useEffect(() => {
    let engine: import("@dot-agent/kernel-dsl").AgentDSLKernel;

    import("@dot-agent/kernel-dsl").then(async (mod) => {
      await mod.init();
