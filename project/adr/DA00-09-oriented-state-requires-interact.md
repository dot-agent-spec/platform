...
Fixing the two examples exposed a question the language has never answered explicitly: what relationship,
if any, does `goal`/`guide`/`teach` have with `interact`? Today only half of that is settled, and only in
the linter, not the grammar or the kernel:
...
| `examples/1. Text Summary`, `examples/2. Fridge Assistant` | Same, except `init`, which carries no orientation at all — a pure router. |
| `apps/dot-agent-cli/templates/agent.behavior` (the `init` scaffold) | Same pattern: `init` is a pure router, `responsive` pairs `goal` + `interact`. |
...
We define an **oriented state** as a state that declares `interact`, exactly once. Only an oriented
state may carry `goal`, `guide`, or `teach`. A state without `interact` is a **router**: it may carry
`on intent`/`on offtopic` handlers (each doing nothing but `transition to`), but no orientation
statements of its own.

This is a **definition, not a grammar change**. The grammar continues to accept the shapes it accepts
today — `state_body` stays a flat, permissive `repeat1(choice(...))`
(`tree-sitter-behavior/grammar.js:105-113`), the same design note there already explains why: "the
linter validates the legal shape per state type." This decision is that validation's missing spec. It
governs every milestone (`DA00-xx`), not one language version, because it is a property of the FSM
model itself, not of any one grammar revision.
...
- **Option B — Require `interact` in every state, no exceptions.** Pro: simplest possible rule, no
  "router" carve-out to explain. Con: contradicts what's already shipped — `init` in `examples/1`,
  `examples/2`, and the CLI's own `templates/agent.behavior` scaffold is a pure router with no
  orientation and no `interact`, and forcing it to pause on entry would be a behavior change to code
  that already works correctly. (rejected)
...
- **Option D (chosen) — Name the oriented-state/router split as a linter-enforced definition, matching
  what every shipped `.behavior` file already does.** Pro: zero behavior change to any passing artifact
  (the four examples and the scaffold already comply); turns a convention four files independently
  arrived at into a citable rule; gives `[platform#15]`(https://github.com/dot-agent-spec/platform/issues/15)'s
  fix, and the helper's own conversion into oriented states, a definition to point at instead of an
  implicit "what the examples do." Con: enforcement doesn't exist yet, so until the follow-up lands the
  rule is convention backed by review, not by tooling — same gap Option A leaves, but named and tracked
  instead of silent.
...
Documentation gains a name to teach with: `dsl-states.md` can say "oriented state" instead of
enumerating what's forbidden, and the router shape (`init` with no orientation, handlers that only
`transition to`) becomes an equally-named, equally-legal alternative rather than an unstated exception.
...
- [platform#15](https://github.com/dot-agent-spec/platform/issues/15) — the doc defect that forced this
  definition; fixed in the same PR that adopts it.
...
- [`apps/dot-agent-cli/helper-src/knowledge/dsl-states.md`](../../apps/dot-agent-cli/helper-src/knowledge/dsl-states.md) —
  where the oriented-state/router split is taught to agent authors.
