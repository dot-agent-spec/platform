...
| ✅ `set` | ✅ `memory_stmt` | ✅ `Statement::Set` (RHS `Value::Path` when unquoted) | | ✅ → `Effect::SetMemory` + writes `MemoryStore`; a path RHS is resolved before the effect is emitted | ✅ `registerHandler("set_memory", fn)` |
| ✅ `if … end` | ✅ `conditional_stmt` | ✅ `Statement::If`; an unquoted operand is `Value::Path` | | ✅ `eval_condition()` resolves at runtime — a path operand reads `MemoryStore` (DA00-10) | ✅ transparent (no effect emitted) |
