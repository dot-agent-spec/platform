...
| **KD-6** | Rename grammar node `temporal_stmt` → `after_stmt` (keyword `after` is canonical; aligns grammar with parser-dsl serde output). Node-name-discrepancy task item 2 (completed, task file removed). | Done — verified in `tree-sitter-behavior/grammar.js` and `node-types.json` |
...
| **AST-1** | node-name-discrepancy | `RunStmt.label` in ast.rs deviates from the canonical grammar field `parameters`. The JSON emits `label`; `parameters` is the standard market term. | Rename `RunStmt.label` → `RunStmt.parameters` in ast.rs; update parser.rs and all consumers (`compiler`, `sdk`). Grammar is not changed. Node-name-discrepancy task item 3 (completed, task file removed). | Done — verified `parameters` present in `ast.rs`, `label` absent |
| **AST-2** | node-name-discrepancy | Serde renames `intent_handler` → `intent_trigger` and `offtopic_handler` → `offtopic_stmt`, diverging from grammar node names. The grammar is canonical. | Change `#[serde(rename = "intent_trigger")]` → `"intent_handler"` and `#[serde(rename = "offtopic_stmt")]` → `"offtopic_handler"` in ast.rs; update all downstream `stmt.type` checks in `compiler/linter.ts`. Node-name-discrepancy task item 4 (completed, task file removed). | Done — verified `intent_handler`/`offtopic_handler` present in `ast.rs`, old names absent |
| **AST-3** | node-name-discrepancy | Field `on_failed` in ast.rs and parser.rs diverges from the grammar canonical field `on_failure` (`parallel_stmt` uses `field('on_failure', $.block)`). Affects `run`, `apply`, `remove`, and `parallel` statement types (4 Rust fields, 4 parser inserts, 1 analysis destructure, 1 linter access). Also exposes a pre-existing bug in `kernel-dsl/fsm.rs:269` (`on_complete` field no longer exists). | Rename `on_failed` → `on_failure` in ast.rs (×4), parser.rs (×4), analysis.rs (×1), linter.ts (×1); fix `kernel-dsl/fsm.rs` Parallel arm. Node-name-discrepancy task item 5 (completed, task file removed). | Done — verified `on_failure` present in `ast.rs`, `on_failed` absent |
