...
| [RFC-0010: .well-known](0010-well-known.md) | Draft | — | — | — | — | — | Defines `/.well-known/dot-agent.json` for publisher-level agent discovery — a catalog only; publisher identity lives in the separate `did.json` that did:web mandates (RFC-0012). Server-side only; no package changes required. |
...
| [RFC-0012: did and proof](0012-identity-proof.md) | Draft | — | — | ⚠️ | — | ⚠️ | Reusable signed-artifact envelope: Ed25519 signature over `aboutme.json` plus author identity via `did:web` (tier 1) or `did:key` (new tier 3, continuity without identity), bound to the `id` namespace by one invariant. Adds optional `agent.did`, parent-signed heredity, and optional publisher pinning in `requires[]`. Compiler must first rework `integrity{}` into a per-file hash chain via `files.json`; SDK `parseAboutme` gains proof verification. RFC-0001/0002/0003 should reference the envelope rather than redefine it. |
