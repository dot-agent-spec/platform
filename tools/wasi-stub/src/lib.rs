...
/// Make an `Id` static.
/// Some WASM toolchains (e.g. wasm32-wasip1 + Rust 1.92+) produce import ids
/// containing `[...]` or `::` that are not valid unquoted WAT identifiers.
/// We fall back to the quoted-identifier form `$"..."` which the WAT spec allows,
/// so that callers in the WAT text can still resolve the stub by name.
fn static_id(id: Option<Id>) -> Option<Id<'static>> {
    id.and_then(|id| {
        let name = id.name().to_owned();
        // Try unquoted first (common case, faster).
        let unquoted = format!("${name}");
        let parser = Box::leak(Box::new(
            wast::parser::ParseBuffer::new(unquoted.leak()).unwrap(),
        ));
        if let Ok(parsed) = wast::parser::parse::<Id>(parser) {
            return Some(parsed);
        }
        // Fall back to quoted identifier for names with chars outside idchar.
        let quoted = format!("$\"{name}\"");
        let parser = Box::leak(Box::new(
            wast::parser::ParseBuffer::new(quoted.leak()).unwrap(),
        ));
        wast::parser::parse::<Id>(parser).ok()
    })
}
